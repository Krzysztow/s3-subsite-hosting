...
export const SUBSITE_DIR = SUBSITE_PREFIX + SUBSITE_DELIMITER;
  ...
  constructor(scope: Construct, id: string, props: SubsiteDeploymentsProps) {
    ...
    const deployment = new s3deploy.BucketDeployment(
      ...
      "SubsiteDeployment",
      {
        ...
        destinationKeyPrefix: SUBSITE_DIR + props.prefix,
      }
    );
    ...
    [
      ...
      s3.EventType.LIFECYCLE_EXPIRATION_DELETE,
      s3.EventType.OBJECT_CREATED,
    ].forEach((eventType) => {
      this.bucket.addEventNotification(
        eventType,
        new s3Notifications.LambdaDestination(updateIndexLambda),
        { prefix: SUBSITE_DIR } // excludes INDEX_DOC (which is generated by lambda); otherwise we'd have infinite looping
      );
    });
